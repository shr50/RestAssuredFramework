...
public class UserTestsUsingPropertyFile {
	...
	public void testPostUser() {
		Response response=UserEndPointsUsingPropertyFile.createUser(userPayload); //response import from io.restassured.response.
	...
	public void testGetUserByName() {
		Response response=UserEndPointsUsingPropertyFile.readUser(userPayload.getUsername());
		response.then().log().all();
		Assert.assertEquals(response.getStatusCode(), 200);
	}
